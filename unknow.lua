-- ============================================================================
-- 👻 KILLER HUB - MM2 ADVANCED VISUAL SUITE (WITH DUELS ESP SUPPORT V4.6)
-- ============================================================================
-- V4.6 Changelog:
--   • Cámara dinámica (Workspace.CurrentCamera reactiva a cambios)
--   • Guard contra doble ejecución
--   • Throttle en la búsqueda de GunDrop (menos CPU)
--   • Limpieza real al Unload (flag + AddTask)
--   • SetDefaults para inputs (sin warns falsos al iniciar)
--   • Config con whitelist (ignora claves desconocidas)
-- ============================================================================
...
local DefaultColors = {
    ...
    DuelEnemy    = Color3_fromRGB(255, 40, 40),
    DuelTeammate = Color3_fromRGB(40, 140, 255)
}
...
-- [2] LOCAL STORAGE SYSTEM (con whitelist: ignora claves desconocidas)
...
-- [3.1] GUARD CONTRA DOBLE EJECUCIÓN
if getgenv().__MM2_Visual_Suite_Active then
    pcall(function() KillerHub:NotifyWarn("Script ya activo", "Ya hay una instancia corriendo.", 3) end)
    return
end
getgenv().__MM2_Visual_Suite_Active = true

-- Defaults para inputs (evita NotifyWarn falso al arrancar por primera vez)
KillerHub:SetDefaults({
    EspMaxDistance = tostring(Config.MaxDistance or 400)
})
...
PagePlayers:CreateMultiDropdown("HighlightFilters", "Roles", {"Murderer","Sheriff","Hero","Innocent","Dead/None"}, function(flags)
    for r in pairs(Config.HighlightRoles) do Config.HighlightRoles[r] = flags[r] == true end; saveConfig()
end)
...
PagePlayers:CreateMultiDropdown("LimbChamsFilters", "Roles", {"Murderer","Sheriff","Hero","Innocent","Dead/None"}, function(flags)
    for r in pairs(Config.LimbChamsRoles) do Config.LimbChamsRoles[r] = flags[r] == true end; saveConfig()
end)
...
PagePlayers:CreateMultiDropdown("BoxFilters", "Roles", {"Murderer","Sheriff","Hero","Innocent","Dead/None"}, function(flags)
    for r in pairs(Config.BoxRoles) do Config.BoxRoles[r] = flags[r] == true end; saveConfig()
end)
...
PagePlayers:CreateMultiDropdown("NameFilters", "Roles", {"Murderer","Sheriff","Hero","Innocent","Dead/None"}, function(flags)
    for r in pairs(Config.NameRoles) do Config.NameRoles[r] = flags[r] == true end; saveConfig()
end)

local ToggleTracer = PagePlayers:CreateToggle("EspTracer", "Tracer ESP", function(val) Config.Tracer = val; saveConfig() end)
PagePlayers:CreateDropdown("EspTracerPos", "Tracer Position",
    {"Bottom Center","Top Center","Middle Left","Middle Right","Center Screen"},
    function(sel) Config.TracerPosition = sel; saveConfig() end,
    Config.TracerPosition or "Bottom Center")
PagePlayers:CreateMultiDropdown("TracerFilters", "Roles", {"Murderer","Sheriff","Hero","Innocent","Dead/None"}, function(flags)
    for r in pairs(Config.TracerRoles) do Config.TracerRoles[r] = flags[r] == true end; saveConfig()
end)
...
local function createRoleColorPicker(page, roleKey, visualName)
    local rgb = Config.CustomColorsRGB[roleKey] or {255, 255, 255}
    local defaultColor3 = Color3_fromRGB(rgb[1], rgb[2], rgb[3])
    ...
    page:CreateToggleColorPicker(
        ...
        function(colorSel)
            Config.CustomColorsRGB[roleKey] = {
                math_floor(colorSel.R * 255),
                math_floor(colorSel.G * 255),
                math_floor(colorSel.B * 255)
            }
            saveConfig()
        end
    )
...
local DistanceInput = PagePlayers:CreateInput("EspMaxDistance", "Max Render Distance (Studs)", "400", function(val)
    ...
    if num then
        Config.MaxDistance = math_abs(num); saveConfig()
    else
        KillerHub:NotifyWarn("Invalid Input", "Please enter numbers only.", 3)
    end
end)
...
PageDuels:CreateMultiDropdown("DuelHighlightFilters", "Teams Filter", {"Enemy","Teammate"}, function(flags)
    for r in pairs(Config.DuelHighlightRoles) do Config.DuelHighlightRoles[r] = flags[r] == true end; saveConfig()
end)
...
PageDuels:CreateMultiDropdown("DuelLimbChamsFilters", "Teams Filter", {"Enemy","Teammate"}, function(flags)
    for r in pairs(Config.DuelLimbChamsRoles) do Config.DuelLimbChamsRoles[r] = flags[r] == true end; saveConfig()
end)
...
PageDuels:CreateMultiDropdown("DuelBoxFilters", "Teams Filter", {"Enemy","Teammate"}, function(flags)
    for r in pairs(Config.DuelBoxRoles) do Config.DuelBoxRoles[r] = flags[r] == true end; saveConfig()
end)
...
PageDuels:CreateMultiDropdown("DuelNameFilters", "Teams Filter", {"Enemy","Teammate"}, function(flags)
    for r in pairs(Config.DuelNameRoles) do Config.DuelNameRoles[r] = flags[r] == true end; saveConfig()
end)
...
PageDuels:CreateMultiDropdown("DuelTracerFilters", "Teams Filter", {"Enemy","Teammate"}, function(flags)
    for r in pairs(Config.DuelTracerRoles) do Config.DuelTracerRoles[r] = flags[r] == true end; saveConfig()
end)
...
-- ============================================================================
-- 🧠 CORE ENGINE (ULTRA-OPTIMIZED V4.6)
-- ============================================================================
...
local function getRoleColor(roleKey, fallbackColor3)
    if Config.CustomColorsActive[roleKey] == true then
        ...
        if rgb then return Color3_fromRGB(rgb[1], rgb[2], rgb[3]) end
    end
...
local function isEspEnabledForRole(espType, status)
    if status == "DuelEnemy" then
        if espType == "Highlight" then return Config.DuelHighlight and Config.DuelHighlightRoles["Enemy"] end
        if espType == "LimbChams" then return Config.DuelLimbChams and Config.DuelLimbChamsRoles["Enemy"] end
        if espType == "Box"       then return Config.DuelBox and Config.DuelBoxRoles["Enemy"] end
        if espType == "Name"      then return Config.DuelName and Config.DuelNameRoles["Enemy"] end
        if espType == "Tracer"    then return Config.DuelTracer and Config.DuelTracerRoles["Enemy"] end
    elseif status == "DuelTeammate" then
        if espType == "Highlight" then return Config.DuelHighlight and Config.DuelHighlightRoles["Teammate"] end
        if espType == "LimbChams" then return Config.DuelLimbChams and Config.DuelLimbChamsRoles["Teammate"] end
        if espType == "Box"       then return Config.DuelBox and Config.DuelBoxRoles["Teammate"] end
        if espType == "Name"      then return Config.DuelName and Config.DuelNameRoles["Teammate"] end
        if espType == "Tracer"    then return Config.DuelTracer and Config.DuelTracerRoles["Teammate"] end
    else
...
local function updatePlayerESP(player)
    ...
    if isEspEnabledForRole("Name", currentStatus) then
        if not nameTag then
            nameTag = Instance_new("BillboardGui")
            nameTag.Name = "KH_Name"
            nameTag.Size = UDim2_new(0, 160, 0, 40)
            nameTag.StudsOffset = Vector3_new(0, 4.0, 0)
...
KillerHub:AddTask(Workspace.ChildAdded:Connect(checkGunInstance))

local function updateGunESP()
    -- Búsqueda throttleada: solo cada GUN_SEARCH_INTERVAL segundos
    if not currentGunDrop or not currentGunDrop:IsDescendantOf(Workspace) then
        local now = os_clock()
        if (now - lastGunSearch) >= GUN_SEARCH_INTERVAL then
            lastGunSearch = now
    ...
    if currentGunDrop and currentGunDrop:IsA("BasePart") and myRoot then
        ...
        if Config.GunName then
            if not nameTag then
                ...
                nameTag.Size = UDim2_new(0, 180, 0, 40)
                nameTag.StudsOffset = Vector3_new(0, 2.5, 0)
                nameTag.AlwaysOnTop = true
                local label = Instance_new("TextLabel")
                label.Name = "Display"
                label.Size = UDim2_new(1, 0, 1, 0)
...
if RoundStart and RoundStart:IsA("RemoteEvent") then
    KillerHub:AddTask(RoundStart.OnClientEvent:Connect(function(arg1, arg2)
        table_clear(playerRoles)
        table_clear(playerDeadStatus)
        currentGunDrop = nil
...
local RoundOver = ReplicatedStorage:FindFirstChild("RoundOver", true)
    or ReplicatedStorage:FindFirstChild("SnowballRoundOver", true)
if RoundOver and RoundOver:IsA("RemoteEvent") then
    KillerHub:AddTask(RoundOver.OnClientEvent:Connect(function()
        isDuelActive = false
        table_clear(playerRoles)
        table_clear(playerDeadStatus)
        table_clear(duelRivals)
        table_clear(duelTeammates)
        currentGunDrop = nil
...
-- DETECCIÓN DE EVENTOS DE DUELOS
if Remotes then
    ...
    if CustomGames and CustomGames:FindFirstChild("DuelStarted") then
        KillerHub:AddTask(CustomGames.DuelStarted.OnClientEvent:Connect(function(duelData)
            table_clear(duelRivals)
            table_clear(duelTeammates)
            isDuelActive = true
            ...
            if type(duelData) == "table" then
                ...
                local myTeamKey = (duelData.Team1 and duelData.Team1[myName]) and "Team1"
                    or ((duelData.Team2 and duelData.Team2[myName]) and "Team2" or nil)

                if myTeamKey then
                    local rivalTeamKey = (myTeamKey == "Team1") and "Team2" or "Team1"
                    if duelData[myTeamKey] then
                        for name in pairs(duelData[myTeamKey]) do
                            if name ~= myName then duelTeammates[name] = true end
                        end
                    end
                    if duelData[rivalTeamKey] then
                        for name in pairs(duelData[rivalTeamKey]) do
    ...
    if Gameplay and Gameplay:FindFirstChild("RoundEndFade") then
        KillerHub:AddTask(Gameplay.RoundEndFade.OnClientEvent:Connect(function()
            isDuelActive = false
            table_clear(duelRivals)
            table_clear(duelTeammates)
        end))
...
Players.PlayerRemoving:Connect(function(player)
    ...
    duelRivals[player.Name] = nil
    duelTeammates[player.Name] = nil
    removeTracerLine(player)
end)

-- MAIN REFRESH LOOP (con bandera de unload)
task.spawn(function()
...
KillerHub:AddTask(RunService.RenderStepped:Connect(function()
...
-- CLEANUP AL CERRAR EL HUB
CoreGui.ChildRemoved:Connect(function(child)
    if child.Name == "KillerHub" and not isUnloaded then
        isUnloaded = true
        pcall(function() GunDrawingLine:Remove() end)
        for _, line in pairs(playerTracers) do
            pcall(function() line:Remove() end)
        end
        table_clear(playerTracers)
        getgenv().__MM2_Visual_Suite_Active = nil
    end
end)
